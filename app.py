    ...
    for engine in [2, 1, 3]:
        for lang in ["tha", "eng"]:
            try:
                ...
                payload["base64Image"] = f"data:image/jpeg;base64,{b64}"
                ...
                if is_thai(text):
                    ...
                    return text, translated, "泰文"
                else:
                    translated = translate(text, "en", "zh-TW")
                    return text, translated, "英文"
            except Exception:
                continue
...
def handle_image(event):
    with ApiClient(configuration) as api_client:
        ...
        try:
            ...
            original, translated, lang = ocr_and_translate(image_data)
            if original is None:
                reply_text = "⚠️ 無法辨識圖片文字，請確認圖片清晰且含有泰文或英文。\n\n小提示：光線充足、文字清楚的圖片辨識效果最好！"
            else:
                short_orig = original[:300] + "..." if len(original) > 300 else original
                reply_text = f"🔍 辨識到{lang}：\n{short_orig}\n\n📖 中文翻譯：\n{translated}"
        except Exception:
